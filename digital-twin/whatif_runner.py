...
class WhatIfRunner:
    ...
    def _simulate(self, state: "WorldState",
        ...
        def tick_process(env):
            for i in range(horizon_ticks):
                ...
                # Update loads from dataset — but don't let an active fault
                # cloned from the live state (pinned_loads / synthetic_faults)
                # get silently overwritten. Mirrors main.py's tick loop so a
                # baseline forecast still reflects an ongoing incident instead
                # of showing it vanish on the very first simulated tick.
                sf_cells = {
                    cid for sf in state.synthetic_faults.values()
                    for cid in sf.get("cells", [])
                }
                for cid, cell in state.cells.items():
                    if cid not in state.pinned_loads and cid not in sf_cells:
    ...
    def _apply_action(self, state: "WorldState", action_plan: dict) -> None:
        ...
        if action_type == "apply_slice_policy":
            ...
            # Mirrors the live /actions/apply_slice_policy endpoint, which
            # treats a slice-policy change as resolving the ephemeral
            # congestion fault — otherwise the "with action" forecast would
            # keep showing the same pinned-high load as the baseline.
