...
| POST | `/fault/restore-agent` | Clear an ephemeral agent fault |
| GET | `/changes` | Every change record applied to the twin, newest first |
| GET | `/faults` | Everything currently deviating from the nominal baseline |
| GET | `/ues` | UE distribution per cell and per slice |
| WS | `/ws/telemetry` | Live telemetry stream, one frame per simulation tick |

### `/ws/telemetry`

A WebSocket that emits one JSON frame whenever the tick counter advances, plus
a `heartbeat` frame every 20 idle seconds. The simulation runs in a plain
thread, so each socket watches the tick counter rather than being pushed to
from that thread.

| Field | Meaning |
|---|---|
| `type` | `snapshot` on connect, then `tick`, or `heartbeat` |
| `tick`, `sim_time_s`, `tick_interval_s`, `ts` | Clock state |
| `kpis` | Latest KPI per cell (same shape as `/metrics`) |
| `cells`, `slices`, `backhaul` | Current entity state |
| `events` | Last 30 network events |
| `pinned_loads`, `synthetic_faults` | Active fault surface |

---

## Twin Console (React UI)

`ui/` holds a React + TypeScript operator console for this twin — live
topology, per-cell drill-down, slice and mobility views, fault injection and
what-if forecasting. It talks only to this service, and embeds the existing
Grafana dashboards on its Analytics page rather than replacing them.

```bash
docker compose up --build -d      # console on http://localhost:8080
make ui-dev                       # or Vite dev server on :5173 against a running stack
```

See `ui/README.md` for the design system and page-by-page detail.
